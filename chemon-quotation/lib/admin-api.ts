...
export async function getUsers(
  ...
  return apiFetch<PaginatedResult<AdminUser>>(`/api/admin/users?${query}`);
}
...
export async function resetUserPassword(
  ...
  return apiFetch<{ tempPassword: string }>(`/api/admin/users/${id}/reset-password`, {
    method: 'POST',
  });
}
...
export async function getActivityLogs(
  ...
  const query = buildQueryString(filters);
  return apiFetch<PaginatedResult<ActivityLog>>(`/api/admin/logs?${query}`);
}
...
export async function getAnnouncements(
  ...
  const query = buildQueryString(filters);
  return apiFetch<PaginatedResult<Announcement>>(`/api/admin/announcements?${query}`);
}
...
export async function getSettings(): Promise<ApiResponse<SystemSettings>> {
  return apiFetch<SystemSettings>('/api/admin/settings');
}
...
export async function getSettingsHistory(
  ...
  return apiFetch<SettingChange[]>(`/api/admin/settings/history?limit=${limit}`);
}
...
export async function getBackups(
  ...
  const query = buildQueryString(filters);
  return apiFetch<PaginatedResult<Backup>>(`/api/admin/backups?${query}`);
}
...
export async function deleteBackup(id: string): Promise<ApiResponse<void>> {
  return apiFetch<void>(`/api/admin/backups/${id}`, {
    method: 'DELETE',
  });
}
...
export async function getSalesStats(
  ...
  const query = buildQueryString(filters);
  return apiFetch<SalesStats>(`/api/admin/stats/sales?${query}`);
}
...
export async function updateUserPermissions(
  ...
  return apiFetch<AdminUserWithPermissions>(`/api/admin/users/${id}/permissions`, {
    method: 'PATCH',
