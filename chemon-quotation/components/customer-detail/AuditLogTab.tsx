    ...
    <div className="bg-[#FAF2E9] rounded-xl">
      <div className="p-6 pb-3">
        <div className="flex items-center justify-between">
          <h3 className="text-[11px] font-bold uppercase tracking-widest text-slate-500">변경 이력</h3>
          <div className="flex items-center gap-2">
            <Select value={view} onValueChange={(v) => setView(v as 'audit' | 'lifecycle')}>
              <SelectTrigger className="w-32 h-8 text-xs bg-white border-none rounded-xl"><SelectValue /></SelectTrigger>
              <SelectContent>
            ...
            {view === 'audit' && (
              <Input
                ...
                className="w-32 h-8 text-xs bg-white border-none rounded-xl focus:ring-2 focus:ring-primary/40"
              />
            )}
          </div>
        </div>
        ...
        {loading ? (
          ...
          auditLogs.length === 0 ? (
            ...
            <div className="space-y-3">
              {auditLogs.map(log => (
                <div key={log.id} className="flex items-start gap-3 rounded-xl bg-[#F5EDE3] p-3">
                  <History className="h-4 w-4 text-slate-500 mt-0.5 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 text-xs text-slate-500 mb-1">
                      <span className="font-medium text-slate-900">{log.changedBy}</span>
                      <span>·</span>
          ...
          lifecycle.length === 0 ? (
            <div className="text-center py-8 text-slate-500 text-sm">라이프사이클 이력이 없습니다</div>
          ) : (
            <div className="relative pl-6">
              <div className="absolute left-2 top-0 bottom-0 w-0.5 bg-[#F5EDE3]" />
              {lifecycle.map((entry, i) => (
                <div key={entry.id} className="relative mb-4 last:mb-0">
                  <div className="absolute -left-4 top-1 h-3 w-3 rounded-full border-2 border-primary bg-[#FAF2E9]" />
                  <div className="rounded-xl bg-[#F5EDE3] p-3">
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">by {entry.triggeredBy}</p>
                  </div>
                </div>
              ))}
            </div>
