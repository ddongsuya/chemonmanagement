    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Input
          ...
          className="w-full sm:w-64 bg-white border-none rounded-xl focus:ring-2 focus:ring-primary/40"
          disabled={loading}
        />
        ...
        {activeFilters.length > 0 && (
          <Button variant="ghost" size="sm" onClick={resetAll} className="text-slate-500">초기화</Button>
        )}
      </div>
      ...
      {/* 확장 필터 */}
      {expanded && (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 bg-[#F5EDE3] rounded-xl p-4">
          <div>
            <Label className="text-[11px] font-bold uppercase tracking-widest text-slate-500 mb-1.5 block">등급</Label>
            <Select value={filters.grade || '_all'} onValueChange={(v) => updateFilter('grade', v === '_all' ? undefined : v)}>
              <SelectTrigger className="bg-white border-none rounded-xl"><SelectValue placeholder="전체" /></SelectTrigger>
              <SelectContent>
          ...
          <div>
            <Label className="text-[11px] font-bold uppercase tracking-widest text-slate-500 mb-1.5 block">세그먼트</Label>
          ...
          <div>
            <Label className="text-[11px] font-bold uppercase tracking-widest text-slate-500 mb-1.5 block">건강도 범위: {filters.healthScoreMin ?? 0} ~ {filters.healthScoreMax ?? 100}</Label>
            <Slider
          ...
          <div>
            <Label className="text-[11px] font-bold uppercase tracking-widest text-slate-500 mb-1.5 block">데이터 품질 범위: {filters.dataQualityMin ?? 0} ~ {filters.dataQualityMax ?? 100}</Label>
            <Slider
              min={0} max={100} step={5}
          ...
          <div>
            <Label className="text-[11px] font-bold uppercase tracking-widest text-slate-500 mb-1.5 block">최근 활동 (일 이내)</Label>
          ...
          <div>
            <Label className="text-[11px] font-bold uppercase tracking-widest text-slate-500 mb-1.5 block">태그 (쉼표 구분)</Label>
            <Input
              placeholder="태그1, 태그2"
