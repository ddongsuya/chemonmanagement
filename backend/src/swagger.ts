      ...
      schemas: {
        ...
        User: {
          ...
          properties: {
            ...
            department: { type: 'string', enum: ['BD1', 'BD2', 'SUPPORT'], nullable: true, description: '부서 (BD1: 사업개발1센터, BD2: 사업개발2센터, SUPPORT: 사업지원팀)' },
            position: { type: 'string', enum: ['STAFF', 'SENIOR', 'ASSISTANT', 'MANAGER', 'DEPUTY', 'GENERAL', 'DIRECTOR', 'CEO', 'CHAIRMAN'], nullable: true, description: '직급' },
        ...
        Customer: {
          ...
          properties: {
            ...
            address: { type: 'string' },
            notes: { type: 'string' },
            grade: { type: 'string', enum: ['LEAD', 'PROSPECT', 'CUSTOMER', 'VIP', 'INACTIVE'], description: '고객 등급' },
            createdAt: { type: 'string', format: 'date-time' },
            updatedAt: { type: 'string', format: 'date-time' },
          },
        },
        ...
        Quotation: {
          ...
          properties: {
            id: { type: 'string', format: 'uuid' },
            quotationNumber: { type: 'string', example: '26-02-DL-0001', description: '견적번호 (형식: YY-MM-UC-NNNN, 시험유형 공통)' },
        ...
        Notification: {
          ...
          properties: {
            id: { type: 'string', format: 'uuid' },
            userId: { type: 'string', format: 'uuid' },
            type: { type: 'string', enum: ['ANNOUNCEMENT', 'SYSTEM', 'REMINDER'], description: '알림 유형' },
            title: { type: 'string' },
            message: { type: 'string' },
            isRead: { type: 'boolean' },
            link: { type: 'string' },
        ...
        Lead: {
          ...
          properties: {
            ...
            source: { type: 'string', enum: ['EMAIL', 'PHONE', 'WEBSITE', 'REFERRAL', 'EXHIBITION', 'OTHER'], description: '유입 경로' },
            status: { type: 'string', enum: ['NEW', 'CONTACTED', 'QUALIFIED', 'PROPOSAL', 'NEGOTIATION', 'CONVERTED', 'LOST', 'DORMANT'], description: '리드 상태' },
        ...
        Contract: {
          ...
          properties: {
            ...
            status: { type: 'string', enum: ['NEGOTIATING', 'SIGNED', 'TEST_RECEIVED', 'IN_PROGRESS', 'COMPLETED', 'TERMINATED'], description: '계약 상태' },
            createdAt: { type: 'string', format: 'date-time' },
          },
        },
        Study: {
          ...
          properties: {
            ...
            species: { type: 'string' },
            status: { type: 'string', enum: ['REGISTERED', 'PREPARING', 'IN_PROGRESS', 'ANALYSIS', 'REPORT_DRAFT', 'REPORT_REVIEW', 'COMPLETED', 'SUSPENDED'], description: '시험 상태' },
            startDate: { type: 'string', format: 'date' },
